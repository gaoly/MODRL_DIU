...
tester_params = {
    ...
    'model_load': {
        'path': './result/20260215_211042_train_fjsp_n100',  # directory path of pre-trained model and log files saved.
...
def main(n_sols = 105):
    ...
    #                 directory = './data/fjs/fjs_n%d_m%d_o%d/'%(j,m,o)
    ...
    #
    #                 directory_pt = './data/pt/pt_n%d_m%d_o%d/' % (j, m, o)
    #                 # 检查目录是否存在，不存在则创建
    #                 if not os.path.exists(directory_pt):
    #                     os.makedirs(directory_pt)
    #                 torch.save(shared_problem, './data/pt/pt_n%d_m%d_o%d/shared_problem_n%d_m%d_o%d_i%d.pt'%(j,m,o,j,m,o,instance_idx+1))
    #                 torch.save(arrival_time, './data/pt/pt_n%d_m%d_o%d/arrival_time_n%d_m%d_o%d_i%d.pt'%(j,m,o,j,m,o,instance_idx+1))
    #                 torch.save(due_time, './data/pt/pt_n%d_m%d_o%d/due_time_n%d_m%d_o%d_i%d.pt'%(j,m,o,j,m,o,instance_idx+1))
    ...
    uniform_weights = uniform_weights.to('cuda:0')

    problem_size_list = [30,100,200]
    #problem_size_list = [30]
    operation_size_list = [5,10,15]
    ...
    for j in problem_size_list:
        for o in operation_size_list:
            env_params['problem_size']=j
            model_params['problem_size'] = j
            env_params['operation_size'] = o
            model_params['operation_size'] = o
            tester = Tester(env_params=env_params,
                            model_params=model_params,
                            tester_params=tester_params,
                            PS=PS,
                            ...
                            EC_IDLE=EC_IDLE)
            for idx in range(5):
                shared_problem = torch.load('./data2/pt/pt_n%d_m%d_o%d/shared_problem_n%d_m%d_o%d_i%d.pt'%(j,machine_size,o,j,machine_size,o,idx+1)).to('cuda:0')
                shared_problem[:, :, 0:5, 0:5] = shared_problem[:,:,0:5,0:5].mean(dim=-1).unsqueeze(3).repeat(1,1,1,machine_size) / PS
                arrival_time = torch.load('./data2/pt/pt_n%d_m%d_o%d/arrival_time_n%d_m%d_o%d_i%d.pt'%(j,machine_size,o,j,machine_size,o,idx+1)).to('cuda:0')
                due_time = torch.load('./data2/pt/pt_n%d_m%d_o%d/due_time_n%d_m%d_o%d_i%d.pt'%(j,machine_size,o,j,machine_size,o,idx+1)).to('cuda:0')
                timer_start = time.time()
                for i in range(n_sols):
                    pref = uniform_weights[i]
                    timer_start = time.time()
                    print(pref)
                    score = tester.run(shared_problem, pref, arrival_time, due_time)
                    sols[i] = np.array(score)
                    timer_end = time.time()
                    total_time = timer_end - timer_start
                    print(total_time*105)
                timer_end = time.time()
                total_time = timer_end - timer_start
                result = get_pareto_solutions(sols)
                df = pd.DataFrame(result)
                directory_sols = './sols3/new_model/sols_n%d_m%d_o%d/' % (j,machine_size,o)
                # 检查目录是否存在，不存在则创建
                if not os.path.exists(directory_sols):
                    os.makedirs(directory_sols)
                df.to_csv("./sols3/new_model/sols_n%d_m%d_o%d/sols_i%d.csv"%(j,machine_size,o,idx+1), index=False)
                file_path = "./sols3/new_model/sols_n%d_m%d_o%d/sols_i%d.csv"%(j,machine_size,o,idx+1)
