...
def SD2_instance_generator(config):
    ...
    arrival_time = np.zeros(n_j)
    temp = np.random.rand(n_j-config.n_ij)
    temp = np.sort(temp)
    temp = temp * (np.mean(op_pt) * n_j * n_op / n_m)
    arrival_time[config.n_ij:] = temp
    EL = np.random.randint(low=1, high=4, size=(n_j)) * 0.5 + 0.2

    due_time = (np.sum(np.mean(op_pt,axis=-1).reshape((n_j,-1)),axis=-1) * EL) + arrival_time
    ...
    return job_length, op_pt, op_per_mch, arrival_time , EL, due_time


def matrix_to_text(job_length, op_pt, op_per_mch, arrival_time , EL, due_time):
    """
...
def text_to_matrix(text):
    ...
    op_pt = []
    numbers = [float(num) for num in text[1].split()]
    EL = []
    arrival_time = []
    due_time = []
    ...
    for j in range(n_j):
        idx = j*3
        EL.append(numbers[idx])
        arrival_time.append(numbers[idx+1])
        due_time.append(numbers[idx+2])
    EL = np.array(EL)
    arrival_time = np.array(arrival_time)
    due_time = np.array(due_time)
    for i in range(n_j):
        content = np.array([int(s) for s in re.findall(r'\d+\.?\d*', text[i + 2])])
    ...
    return job_length, op_pt, arrival_time , EL, due_time
...
def load_data_from_files(directory):
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for f in files:
            ...
            job_length, op_pt, arrival_time , EL, due_time = text_to_matrix(g)
            dataset_job_length.append(job_length)
            dataset_op_pt.append(op_pt)
            dataset_arrival_time.append(arrival_time)
            dataset_EL.append(EL)
            dataset_due_time.append(due_time)
    return dataset_job_length, dataset_op_pt, dataset_arrival_time, dataset_EL, dataset_due_time
...
def generate_data_to_files(seed, directory, config):
    ...
    if (not os.path.exists(path)) or config.cover_data_flag:
        ...
        for idx in range(batch_size):
            if source == 'SD2':
                job_length, op_pt, op_per_mch, arrival_time , EL, due_time = SD2_instance_generator(config=config)
                print(job_length)
                print(job_length.shape)
                print(aaaaaaaaaaa)
                lines_doc = matrix_to_text(job_length, op_pt, op_per_mch, arrival_time , EL, due_time)
